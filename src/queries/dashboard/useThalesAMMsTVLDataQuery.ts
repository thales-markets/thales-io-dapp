...
const useThalesAMMsTVLDataQuery = (options: UseQueryOptions<AMMsTVLData | undefined>) => {
    return useQuery<AMMsTVLData | undefined>(QUERY_KEYS.AMM.ThalesAMMsTVLData(), thalesAMMsTVLDataQueryFn, {
        ...options,
    });
};

export const thalesAMMsTVLDataQueryFn = async () => {
    ...
    try {
        ...
        const [opLiquidityPoolData, arbLiquidityPoolData, baseLiquidityPoolData] = await Promise.all([
            opThalesLPDataContract.getLiquidityPoolData(thalesLiquidityPoolContract.addresses[Network.OptimismMainnet]),
            arbThalesLPDataContract.getLiquidityPoolData(thalesLiquidityPoolContract.addresses[Network.Arbitrum]),
