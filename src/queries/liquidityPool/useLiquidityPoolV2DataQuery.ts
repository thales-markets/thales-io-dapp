...
const useLiquidityPoolV2DataQuery = (
    address: string,
    collateral: Coins,
    networkId: Network,
    ...
    return useQuery<LiquidityPoolData | undefined>(
        QUERY_KEYS.LiquidityPoolV2.Data(address, networkId),
        async () => {
            ...
            try {
                ...
                if (liquidityPoolDataV2Contract) {
                    const contractLiquidityPoolData = await liquidityPoolDataV2Contract.getLiquidityPoolData(address);
...
export default useLiquidityPoolV2DataQuery;
