        ...
        if (thalesTokenContract && celerBridgeContract) {
            ...
            try {
                ...
                if (txResult && txResult.transactionHash) {
                    toast.update(
                        id,
                        getSuccessToastOptions(t('bridge.history.confirm-refund-button.confirmation-message'), id)
                    );
    ...
    return (
        <Container>
            ...
            >
                ...
                >
                    {!isSubmitting
                        ? t('bridge.history.confirm-refund-button.label')
                        : t('bridge.history.confirm-refund-button.progress-label')}
                </Button>
                {isNetworkSwitchDropDownOpen && (
                    <NetworkSwitchDropDown>
                        {t('bridge.history.switch-message', {
                            network: SUPPORTED_NETWORK_IDS_MAP[srcChainId]?.name,
                        })}
                        <Button onClick={handleSwitch} {...defaultSwitchButtonProps}>
                            {t('bridge.history.switch-label')}
                        </Button>
                    </NetworkSwitchDropDown>
                )}
            </OutsideClickHandler>
